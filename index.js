...
const config = {
  ...
  TITLE_CACHE_TTL: 604800000, // 1 week
...
const handleDownload = async (req, res, format) => {
  ...
  try {
    ...
    if (error.stderr?.includes('Video unavailable')) {
      errorMessage = 'Video unavailable (private, removed, or age-restricted)';
    } else if (error.signal === 'SIGTERM') {
      errorMessage = 'Download terminated';
    }
...
app.get('/download-progress', (req, res) => {
  ...
  activeDownloads++;
  logger.info(`Starting progress tracking for ${id}`);
  ...
  child.on('close', (code, signal) => {
    ...
    if (code === 0 && fs.existsSync(outputPath)) {
      ...
      const errorMsg = signal === 'SIGTERM' ? 'Download terminated' : 'Download failed';
